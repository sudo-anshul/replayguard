...
subprocess.run([sys.executable, str(ROOT/'infra/build_template.py')], cwd=ROOT, check=True)
...
subprocess.run(base + ['cloudformation','deploy','--stack-name',a.stack,'--template-file',str(ROOT/'infra/template.json'),'--capabilities','CAPABILITY_IAM','--parameter-overrides',f'LabName={a.stack}',f'LabExpiresAt={expiry}',f'ReservedConcurrency={reservation}','--tags','Project=ReplayGuard','Purpose=FirstCommitFailureLab','--no-fail-on-empty-changeset'], check=True)
...
(ROOT/'deployment.local.json').write_text(json.dumps(outputs,indent=2)+'\n')
print('Deployment ready. Run: python scripts/run_case.py --stack '+a.stack+' --region '+a.region+(' --profile '+a.profile if a.profile else ''))
