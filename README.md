...
The engine has 26 focused tests. Nine native edge reports passed 70 native/browser parity checks and a separate browser import pass, including wrong payloads, repeated reuse, timeouts, and JSON numeric equality. The [results and limits](docs/repair-lab/results.md) link the evidence. [Clean kit validation](docs/repair-lab/validation/README.md) executes a copied repair, changes it to `key=None`, observes a violation, restores it, and verifies a pass with networking denied.
